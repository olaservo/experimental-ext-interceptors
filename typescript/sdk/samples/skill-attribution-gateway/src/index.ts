...
import { skillAttributionValidator } from './attributionValidator.js';

const DEFAULT_PORT = 39818;
...
function buildServer(): Server {
  const server = new Server(
    { name: 'skill-attribution-gateway', version: '0.1.0' },
    { capabilities: {} },
  );
  withInterceptors(server, { interceptors: [skillAttributionValidator] });
  return server;
}
...
async function main(): Promise<void> {
  ...
  process.stderr.write(
    `[skill-attribution-gateway] listening on http://localhost:${port}${path}\n`,
  );
